...
Сервис работает в режиме **in-memory only**. Данные не сохраняются на диск и сбрасываются при рестарте процесса/пересоздании контейнера.
...
Для pool-ключей (`|bidPool` / `|askPool`) хранится только последний объект метаданных пула.
...
# Запись метаданных пула (object value, timestamp игнорируется)
curl -X POST http://localhost:3002/store/write \
  -H 'Content-Type: application/json' \
...
src/
  ...
  store/
    ...
    store.controller.ts            # REST-эндпоинты + Swagger декораторы
